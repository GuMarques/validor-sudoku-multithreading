...
public class ThreadRow extends Thread {
    ...
    public ThreadRow(int index, Validator validator) {
        if(index < 0 || index > 8)
    ...
    @Override
    public void run() {
        if(!this.game.verifyRow(index)) {
            throw new Error("A linha " + (index + 1) + " não é válida");
        } else {
            System.out.println("A linha " + (index + 1) + " é válida");
        }
    }
}
